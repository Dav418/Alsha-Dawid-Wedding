...
double focusProgressForItem({
  ...
  final itemCenterY =
      topPadding + itemIndex * itemExtent + itemExtent / 2 - scrollOffset;

  final distanceFromFocus = (itemCenterY - focusY).abs();
...
double _delayedProgress(double progress) {
  return ((progress - 0.4) / 0.6).clamp(0.0, 1.0).toDouble();
}
      ...
      child: Stack(
        ...
        children: [
          if (!isFirst)
            const Positioned(
              top: 0,
              bottom: WeddingDayTimeline._rowExtent / 2,
              child: _TimelineRailLine(),
            ),
          if (!isLast)
            const Positioned(
              top: WeddingDayTimeline._rowExtent / 2,
              bottom: 0,
              child: _TimelineRailLine(),
            ),
          Container(
...
class _TimelineCard extends StatelessWidget {
  ...
  static const _collapsedHeight = 106.0;
  static const _expandedHeight = 166.0;

  @override
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;

    final detailsProgress = _delayedProgress(progress);
    final visibleCardHeight =
        _lerp(_collapsedHeight, _expandedHeight, progress);
    final cardOpacity = _lerp(0.36, 1.0, progress);
    final textOpacity = _lerp(0.45, 1.0, progress);
    final imageOpacity = _lerp(0.0, 0.9, detailsProgress);
